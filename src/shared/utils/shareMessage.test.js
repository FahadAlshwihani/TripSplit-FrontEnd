...
describe('English', () => {
  test('open, no password', () => {
    const message = buildTripShareMessage({ t: i18n.t.bind(i18n), tripName: 'summer', url, joinCode, joinPolicy: 'open' });
    expect(message).toBe('Join us on "summer" 👋\nJoin code: ABCD1234\nhttps://example.com/trips/short-1\n\nYou can jump straight in.');
  });

  test('approval_required, no password', () => {
    const message = buildTripShareMessage({ t: i18n.t.bind(i18n), tripName: 'summer', url, joinCode, joinPolicy: 'approval_required' });
    expect(message).toBe('Join us on "summer" 👋\nJoin code: ABCD1234\nhttps://example.com/trips/short-1\n\nOnce you\'re in, let me know so I can approve your request.');
  });

  test('approval_required, with password', () => {
    const message = buildTripShareMessage({ t: i18n.t.bind(i18n), tripName: 'summer', url, joinCode, joinPolicy: 'approval_required', password: 'hunter2' });
    expect(message).toBe('Join us on "summer" 👋\nJoin code: ABCD1234\nhttps://example.com/trips/short-1\n\nPassword: hunter2\n\nOnce you\'re in, let me know so I can approve your request.');
  });

  test('invite_only, no password', () => {
    const message = buildTripShareMessage({ t: i18n.t.bind(i18n), tripName: 'summer', url, joinCode, joinPolicy: 'invite_only' });
    expect(message).toBe('You\'re invited to join "summer" 👋\nHere\'s your invite link:\nhttps://example.com/trips/short-1');
  });

  test('invite_only, with password', () => {
    const message = buildTripShareMessage({ t: i18n.t.bind(i18n), tripName: 'summer', url, joinCode, joinPolicy: 'invite_only', password: 'hunter2' });
    expect(message).toBe('You\'re invited to join "summer" 👋\nHere\'s your invite link:\nhttps://example.com/trips/short-1\n\nPassword: hunter2');
  });
...
describe('Arabic', () => {
  ...
  test('open, no password', () => {
    const message = buildTripShareMessage({ t: i18n.t.bind(i18n), tripName: 'summer', url, joinCode, joinPolicy: 'open' });
    expect(message).toBe('انضم معنا في رحلة "summer" على TripSplit - قطتنا 👋\nكود الانضمام: ABCD1234\nhttps://example.com/trips/short-1\n\nادخل مباشرة وبتنضم للرحلة.');
  });
...
test('the current app locale controls the language, never an independent browser-language check', async () => {
  ...
  const arMessage = buildTripShareMessage({ t: i18n.t.bind(i18n), tripName: 'summer', url, joinCode, joinPolicy: 'open' });
  await i18n.changeLanguage('en');
  const enMessage = buildTripShareMessage({ t: i18n.t.bind(i18n), tripName: 'summer', url, joinCode, joinPolicy: 'open' });
...
test('no raw internal UUID appears in a share message -- callers are always responsible for passing a short_code URL', () => {
  const message = buildTripShareMessage({ t: i18n.t.bind(i18n), tripName: 'summer', url: 'https://example.com/trips/short-1', joinCode, joinPolicy: 'open' });
