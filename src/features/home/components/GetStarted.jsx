...
const GetStarted = () => {
  ...
  return <section className="home-get-started" id="get-started">
    <p className="home-get-started__eyebrow text-label">{t('home.getStarted.eyebrow')}</p>
    <div className="home-container-pc">
      {loading || authLoading ? <Loading /> : <>
        ...
        <div className="card-pc create-trip-card" id="create-trip"><h2>{t('create.new.trip')}</h2><form onSubmit={submitCreate}><input className="pc-input" value={create.title} onChange={(e) => setCreate({ ...create, title: e.target.value })} placeholder={t('Trip.title')} required /><input className="pc-input" type="number" min="0.01" step="0.01" value={create.budget} onChange={(e) => setCreate({ ...create, budget: e.target.value })} placeholder={t('Budget')} required /><select className="pc-input" value={create.currency} onChange={(e) => setCreate({ ...create, currency: e.target.value })}>{['SAR','USD','EUR','GBP','AED','QAR','KWD','BHD','OMR'].map((item) => <option key={item}>{item}</option>)}</select><input className="pc-input" type="password" value={create.password} onChange={(e) => setCreate({ ...create, password: e.target.value })} placeholder="Room password (optional)" />{!user && <GuestFields values={create} onChange={setCreate} />}<button className="pc-btn-create">{t('create.trip.button')}</button></form></div>
        <div className="card-pc join-trip-card" id="join-trip"><h2>{t('join.existing.trip')}</h2><form onSubmit={submitJoin}><input className="pc-input" value={join.join_code} onChange={(e) => setJoin({ ...join, join_code: e.target.value.toUpperCase() })} placeholder={t('enter.trip.code')} required /><input className="pc-input" type="password" value={join.password} onChange={(e) => setJoin({ ...join, password: e.target.value })} placeholder="Room password (if required)" />{!user && <GuestFields values={join} onChange={setJoin} />}<button className="pc-btn-join">{t('join.trip.button')}</button></form></div>
        {error && <div className="error-message" role="alert">{error}</div>}
        {user && <div className="card-pc"><h2>{t('trip.history')}</h2>{Object.entries(tripGroups).map(([status, rows]) => <section key={status}><h3>{t(`trip.group.${status}`)}</h3>{rows.length ? <div className="expenses-list">{rows.map((trip) => <Link className="expense-item trip-history-item" key={trip.id} to={`/trip/${trip.id}`}><span className="member-avatar">{avatarGlyph(user.avatar_key)}</span><span><strong>{trip.title}</strong><br />{trip.currency} {trip.budget} · {trip.member_count} {t('members.title')} · {t('dashboard.yourBalance')} {trip.current_balance}<br /><small>{trip.last_activity_at ? new Date(trip.last_activity_at).toLocaleDateString() : '—'}</small></span></Link>)}</div> : <p>{t(`trip.empty.${status}`)}</p>}</section>)}</div>}
      </>}
    </div>
  </section>;
};
