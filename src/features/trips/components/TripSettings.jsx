...
export const TripSettings = ({ trip, permissions, onUpdate, onArchive, onRestore }) => {
  ...
  // Trip.budget is the explicit, canonical Fund budget target (see
  // docs/architecture/fund-accounting.md) -- always a plain editable
  // field here regardless of Fund/round activity. The only guard is
  // server-side (budget_below_collected: can't drop it below money
  // already confirmed collected) -- creating, cancelling, or completing
  // a FundingRound has no bearing on whether this field is editable.
