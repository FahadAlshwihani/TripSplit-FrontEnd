...
const initialForm = (members, expense, currentMember, categories) => {
  if (expense) return {
    title: expense.title, amount: expense.amount, category: expense.category, scope: expense.scope || 'shared', expense_date: expense.expense_date,
  ...
  return { title: '', amount: '', category: categories.find((item) => item.code === 'other')?.code || categories[0]?.code || 'other', scope: 'shared', expense_date: today(), notes: '', split_type: 'equal', participant_ids: members.map((m) => m.id), payments: payer ? { [payer.id]: '' } : {}, splitValues: {} };
};

const ExpenseForm = ({ members, categories = DEFAULT_CATEGORIES, currentMember, expense, onSubmit, onCancel, disabled }) => {
  const { t } = useTranslation();
  const [form, setForm] = useState(() => initialForm(members, expense, currentMember, categories));
  useEffect(() => setForm(initialForm(members, expense, currentMember, categories)), [expense, members, currentMember, categories]);
  ...
  const submit = (event) => {
    ...
    const payload = { title: form.title, amount: form.amount, category: form.category, scope: form.scope, expense_date: form.expense_date, notes: form.notes, split_type: form.split_type, payments };
  ...
  const obviousInvalid = form.scope === 'shared' && (!form.participant_ids.length || !Object.keys(form.payments).length || remaining !== 0 || (form.split_type === 'percentage' && splitAssigned !== 100) || (form.split_type === 'exact' && toCents(splitAssigned) !== toCents(form.amount)));
