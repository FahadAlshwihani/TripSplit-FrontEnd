...
const AuthPage = () => {
  ...
  const submitEmail = async (event) => {
    ...
    try { const result = await requestOtp(email); setOtpId(result.otp_id); setStep('otp'); }
  ...
  return <MainLayout><div className="home-container-pc mt-5"><div className="card-pc auth-card">
    ...
    {step === 'otp' && <form onSubmit={submitOtp}><p>We sent a six-digit code to {email}.</p><input className="pc-input otp-input" inputMode="numeric" maxLength="6" pattern="[0-9]{6}" value={code} onChange={(e) => setCode(e.target.value.replace(/\D/g, ''))} required /><button className="pc-btn-create" disabled={busy}>{busy ? 'Checking…' : 'Verify'}</button><button type="button" className="pc-btn-join" onClick={() => setStep('email')}>Use another email</button></form>}
