...
const InvitationPage = () => {
  ...
  const accept = async (payload) => { if (accepting) return; setAccepting(true); try { const result = await acceptInvitation(token, payload); navigate(`/trip/${result.trip.id}`); } catch (err) { setError(err.response?.data?.message || t('invite.invalid')); setAccepting(false); } };
  useEffect(() => { if (invite?.valid && invite.email_required && user && !authLoading) accept({}); }, [invite, user, authLoading]); // resumes after OTP/onboarding
