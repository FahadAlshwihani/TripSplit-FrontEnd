...
`TripSupportPage` (`/trips/:tripId/support`) ports the supplied Stitch Support Center source's page CONTENT into the app's own `DashboardShell` — never Stitch's sidebar/topbar, which stays this app's existing shell. No fetch of its own (trip/`currentMember` come from `TripLayout`'s outlet context, user identity from `useAuth()`), so there is no page-level loading state, matching every other static Settings-style page.

- **One unified Articles/Contact Us tab workspace — identical on desktop and mobile, no modal/drawer**: `activeSupportPanel` (`'articles' | 'contact'`, local `useState` in `TripSupportPage`) is the single source of truth for which panel is visible. The persistent tab nav (`role="tablist"`, two `role="tab"` buttons — Articles/`library_books`, Contact Us/`chat_bubble`) is the only way to switch; there is no modal, drawer, portal, overlay, close button, or "Back to articles" control anywhere in this feature (a static test, `supportIcons.test.js`, guards against `ModalPortal`/`useModalDialog` imports or that string ever reappearing in the workspace files). The active tab gets the exact same indigo hard-shadow "primary" treatment other primary actions in this app use (never an underline-only or text-color-only active state); the right-side workspace holds both `role="tabpanel"` panels (Knowledge Base grid, `SupportForm`) **permanently mounted**, toggled only via the `hidden` attribute — never conditionally rendered in/out of the tree — specifically so a draft the requester is mid-typing in `SupportForm` survives switching to Articles and back.
  - **`SupportForm`** (`features/support/components/SupportForm.jsx`) owns all business logic (identity resolution, field state, validation-error rendering, submit, success state) and has no header chrome of its own beyond a title/helper line — no close button, no back link — since the persistent tab nav is the only navigation in or out of it.
  - **"Report a Problem"** is a secondary CTA beside the tab nav that switches to Contact Us AND forces the subject to `technical_problem`. Because `SupportForm` now stays mounted for the page's whole lifetime (draft preservation), a plain `useState` initializer can't react to a second, later click — `TripSupportPage` instead bumps a `presetSignal` counter (alongside `presetSubject`) on every Report-a-Problem click, and `SupportForm` re-applies the preset in a `useEffect` keyed on that counter, never on a plain "Contact Us" click (which must never overwrite an in-progress draft).
  - After a successful submission, the form shows the reference and a "Submit another request" action (clears the draft, stays on Contact Us) — it never auto-navigates back to Articles.
  - It POSTs to `POST /api/v1/support/tickets/` (`features/support/api/supportApi.js`) via `createSupportTicket(tripId, payload)`, which attaches the per-trip guest token (`tripRequest(tripId)`) exactly like every other trip-scoped API call — no new guest-auth mechanism. A registered requester's name/email are read-only, sourced from the existing `useAuth()` context (never re-fetched); a guest's name is read-only from `currentMember`, and only their email — which has no canonical source — is an editable required field. The server independently re-derives a registered user's identity from the session regardless of what the form sends; this is a display courtesy, not the actual trust boundary.
  - `ModalPortal`/`useModalDialog` are unaffected everywhere else in the app (e.g. `SettlementTimelineDrawer`) — Support simply no longer depends on them.
- **Phone**: `src/shared/components/PhoneField.jsx` is one coherent bordered control (country-code `<select>` + number `<input>`), never two unrelated fields. `src/shared/data/countryCallingCodes.js` is a curated Gulf/MENA-first list (same "verified priority list, not exhaustive ITU-T coverage" approach as `currencySearchAliases.js`), not a claim of full telecom validation — the backend's own plausibility checks (see the backend repo's `apps/support/serializers.py`) are the actual validation.
- **Knowledge Base**: four categories (`getting-started`, `expenses`, `fund`, `settlements`) are real content, not decorative cards — each routes to `/trips/:tripId/support/:category` (`SupportArticlePage`, a sibling leaf route of `support` in `tripRoutes.jsx`, matching the app's flat trip-route convention; no nested `<Route>` children were introduced). Content lives in `features/support/content/articles.en.js` / `articles.ar.js` — frontend-local, structured, bilingual data (`{title, icon, intro, sections: [{id, heading, paragraphs, bullets?, steps?, callout?}]}`), not a CMS and not i18n JSON (the flat locale-file convention doesn't suit long-form nested content). `SupportArticlePage` is one generic renderer for all four categories, with a table-of-contents, section anchors, and a "related pages" block linking back into the trip using whatever identifier (`short_code` in real usage) is already in the current URL — never a separately-sourced UUID.
- **Material Symbols on this route**: `support.css` carries its own `@import url('https://fonts.googleapis.com/css2?family=Material+Symbols+Outlined...')`, matching `settings.css`/`governance.css`/`settlements.css` byte-for-byte (see `styles/supportIcons.test.js`, a regression guard). An earlier version of this file had the `.material-symbols-outlined` font-family rule but not the `@import` that actually loads the font — every icon rendered as raw ligature text on a fresh visit to `/support` (the common case; only looked fine if the browser had already loaded one of those three other pages first in the same session, whose own `@import` happened to still be registered). Every icon in this feature renders through that one working `.material-symbols-outlined` primitive; the one remaining directional glyph (`arrow_right_alt`, the Knowledge Base card's "read articles" chevron) gets an explicit `transform: scaleX(-1)` under `[dir="rtl"]`, since the font itself doesn't auto-mirror (the base rule forces `direction: ltr` for correct ligature resolution, which also freezes the glyph's own visual direction).
