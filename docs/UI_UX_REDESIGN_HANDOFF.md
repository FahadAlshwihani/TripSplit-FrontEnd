...
- **Contact form**: `SupportContactDrawer` (`features/support/components`) is the ONE canonical form both "Contact Support" (no preselected subject) and "Report a Problem" (preselects `technical_problem`, still changeable) open — never a second copy. It POSTs to `POST /api/v1/support/tickets/` (`features/support/api/supportApi.js`) via `createSupportTicket(tripId, payload)`, which attaches the per-trip guest token (`tripRequest(tripId)`) exactly like every other trip-scoped API call — no new guest-auth mechanism. A registered requester's name/email are read-only, sourced from the existing `useAuth()` context (never re-fetched); a guest's name is read-only from `currentMember`, and only their email — which has no canonical source — is an editable required field. The server independently re-derives a registered user's identity from the session regardless of what the form sends; this is a display courtesy, not the actual trust boundary.
- **Drawer = desktop side panel AND mobile modal, one component**: `support.css`'s `.support-drawer*` is its own local copy of the app's one existing drawer recipe (`expenses.css`'s `.exp-drawer*` — same "each lazy-loaded route chunk carries its own copy" convention as `settings.css`/`governance.css`). It's a fixed panel sliding from `inset-inline-end` that collapses to a full-viewport sheet under 480px, so it's already both "desktop side panel, not a centered modal" and "mobile canonical modal shell" — no separate mobile implementation exists or was needed. Uses `ModalPortal` + `useModalDialog` (focus trap, Escape, focus return) — the same primitives every other dialog in the app already uses.
